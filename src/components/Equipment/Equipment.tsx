const Equipment = () => {
  const equipments = [
    { name: "3D CT (Eco-X)", desc: "저선량 고해상도 3차원 영상 제공. 방사선 피폭량을 획기적으로 줄이면서도 정밀한 진단 가능" },
    { name: "턱관절 물리치료기", desc: "턱관절 장애, 임플란트, 사랑니 발치 시 세포재생, 통증감소 및 염증완화에 효과적" },
    { name: "iTero 구강스캐너", desc: "3D 치아 이미지 구현으로 인비절라인 치료에 효과적이며 편안한 인상 채득 가능" },
    { name: "Prime 구강스캐너", desc: "굴곡진 부분, 가파른 부분의 정밀한 인상 채득이 가능해 정교한 보철물 제작" },
    { name: "무통마취기", desc: "마취제 주입 속도와 압력을 전자동으로 조절해 보다 빠르고 효과적인 통증 완화" },
    { name: "산소포화도 측정기", desc: "수술 및 발치 시 환자의 혈액 내 산소 농도를 실시간으로 모니터링하여 안전 확보" },
    { name: "집진기", desc: "치아 그라인딩 시 발생하는 미세 분진을 효과적으로 포집하여 쾌적한 환경 유지" },
    { name: "미백치료기", desc: "LED 광선을 이용하여 미백제의 활성화를 돕는 시술로 저자극 고효율 치아 미백" },
    { name: "무선 광중합기", desc: "충치/심미치료 시 빠르고 균일한 조사로 재료의 강도와 모양을 정확하게 형성" },
    { name: "고압증기멸균", desc: "모든 진료 도구와 기구에 대한 철저한 멸균소독을 통해 감염 걱정 없는 안전한 치료" },
  ];
           ...
           {equipments.map((item, index) => (
             <div key={index} className="bg-gray-800 p-6 rounded-xl hover:bg-gray-700 transition duration-300 group cursor-pointer">
               <div className="aspect-square bg-gray-700 rounded-lg mb-4 relative overflow-hidden">
                  {/* Image Placeholder */}
                  <div className="absolute inset-0 bg-gradient-to-br from-gray-600 to-gray-800 group-hover:scale-105 transition duration-500"></div>
